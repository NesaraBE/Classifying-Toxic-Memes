...
The meme dataset was obtained from Meta’s AI Hateful Meme Challenge Dataset.  The memes are pre labeled as 0 (not hateful) or 1 (hateful).  Training set includes 8500 memes and the testing set includes 1000.  Each meme contains a single image and caption.  For clustering, only the training images are used.  External measures are based on the same 0 or 1 binary hate classification. 

## Methods


To create a multimodal model, we used several pre-trained models to extract features from the text and image portions of the memes which we then combined using various fusion techniques. The pre-trained models used include BERT, ResNet50 and CLIP, a “zero-shot” network [7]. Feature extraction from captions was performed using BERT, a pre-trained neural network to get sentence vectors [5]. Similarly, feature extraction from images was performed using ResNet50, another pre-trained neural network [6]. The layer outputs of both neural networks were saved at each stage of computation to be used in our input fusion process.

To investigate the success of various fusion techniques, we used features extracted from several stages of computation in the pre-trained models. We have 6 possible fusion combinations in our case:
...
Once we extracted the features from these models, we designed our own neural network to fuse both the image and text features and trained it on the Hateful Memes Dataset. Our neural network was composed of multiple dense layers, with dropout and skip connections like those used in ResNet to allow for faster training and to reduce overfitting. Different fusion techniques are tried and the results are discussed in the next section. 
...
Next, we employ a technique similar to bagging for fusing different layer features. Features are sampled randomly from the 13 layers of BERT and three features are chosen with replacement. For the image, only the last layer output is taken. The sampling is repeated 10 times to obtain various concatenated features. The model then trains the same network architecture on different combinations of features. Instead of a hard threshold and majority voting, the average of sigmoid outputs is obtained to find the final output for each model on the validation data. The results are discussed in the next section.

In addition to these two networks, CLIP was used to generate these features as well as provide a baseline for classifying memes as hateful or not-hateful. CLIP is an image classifying network that was trained on a very diverse set of images, allowing it to accurately classify images from many datasets without re-training. In this project CLIP was modified to classify the memes as hateful or not-hateful (Table 5). It was also used to extract image and text features from internal image and text encoders. CLIP has multiple internal image encoders, the ones used for this project are a modified ResNet50 model and a custom vision transformer. They provide a single internal text encoder which is a custom transformer-based model. We include these encoders in our project to examine the effect of encoder choice on the ability to classify memes.

Thus, we next considered a supervised fusion model to improve accuracy by incorporating training labels.  To extract more meaningful features, we used CLIP, a powerful pre-trained neural network reliant on zero-shot transfer and multimodal learning, to learn our dataset.  Performance was measured against ResNet50 using accuracy and area under the receiver operating characteristic (AUROC) curves.  Overfitting issues were corrected using dropout layers and skip connections, which also mitigated the issue of vanishing gradients during backward propagation. Finally, technique similar to bagging is employed to find relevant layers of different architectures.
